...
import { 
  Sparkles, Cpu, Zap, Brain, Globe, TrendingUp, BarChart3,
  MessageSquare, Users, Target, Bell, Search, Menu,
  ChevronRight, ChevronDown, ArrowUpRight, MoreHorizontal,
  Star, CheckCircle2, AlertTriangle, Radio, Command,
  Layers, Settings, Workflow, Crown
} from "lucide-react";
import Link from "next/link";

const GlowingCard = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => (
  <div className={`relative bg-white/[0.02] border border-white/[0.06] rounded-2xl p-6 hover:bg-white/[0.04] hover:border-purple-500/20 transition-all duration-500 group ${className}`}>
...
export default function DashboardPage() {
  ...
  return (
    <div className="flex h-screen text-white overflow-hidden relative" style={{ background: 'radial-gradient(ellipse at 30% 20%, #0f0c29 0%, #0a0a1a 40%, #020210 100%)' }}>
      ...
      <motion.aside animate={{ width: sidebarOpen ? 280 : 0 }} className="h-full border-r border-white/[0.04] bg-[#060612]/90 backdrop-blur-2xl flex flex-col overflow-hidden z-30">
        <div className="p-6">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 border-b border-white/[0.04] px-6 flex items-center justify-between bg-[#060612]/70 backdrop-blur-2xl shrink-0">
          ...
          <div className="flex items-center gap-3">
            {mounted && <span className="text-[10px] text-white/20 font-mono tracking-wider">{time.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>}
        ...
        <div className="flex-1 overflow-y-auto p-8">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <h1 className="text-3xl font-bold mb-1">Welcome back, Ajay</h1>
            <p className="text-white/30 text-sm font-mono">{'>'} Neural systems operational. {projects.filter(p => p.status === 'active').length} active projects.</p>
          </motion.div>
              ...
              <motion.div key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }}>
                <GlowingCard>
                  <div className="flex items-start justify-between mb-4">
                    <div className="p-2.5 bg-purple-500/10 rounded-xl"><metric.icon className="w-5 h-5 text-purple-400" /></div>
                    <div className={`flex items-center gap-1 text-[10px] ${metric.change > 0 ? 'text-emerald-400' : 'text-red-400'}`}>{metric.change > 0 ? <ArrowUpRight className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}{Math.abs(metric.change)}%</div>
                  </div>
                  <p className="text-3xl font-bold font-mono tracking-tight mb-1">{metric.value}</p>
                  <p className="text-[10px] text-white/30 uppercase tracking-widest">{metric.label}</p>
                  <div className="mt-3"><Sparkline data={sparklineData} /></div>
                </GlowingCard>
              </motion.div>
            ))}
          </div>
              ...
              <GlowingCard>
                ...
                <div className="space-y-3">
                  {projects.map((project, idx) => (
                    <motion.div key={project.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }} className="flex items-center gap-4 p-4 bg-white/[0.01] hover:bg-white/[0.03] rounded-xl border border-white/[0.04] transition-all group">
                      <div className="p-2.5 bg-purple-500/10 rounded-xl"><project.icon className="w-5 h-5 text-purple-400" /></div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1"><span className="font-medium text-sm">{project.name}</span><StatusBadge status={project.status} /></div>
                        <div className="flex items-center gap-3">
                          <div className="flex-1 h-1.5 bg-white/[0.04] rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${project.progress}%` }} transition={{ duration: 1, delay: 0.5 }} className="h-full bg-gradient-to-r from-purple-500 to-cyan-500 rounded-full" /></div>
                          <span className="text-[10px] text-white/30 font-mono">{project.progress}%</span>
                        </div>
                      </div>
                      <div className="text-right"><div className="flex items-center gap-1 mb-1"><Star className="w-3 h-3 text-amber-400" /><span className="text-sm font-bold">{project.aiScore}</span></div><span className="text-[9px] text-white/20">AI Score</span></div>
                      <button className="p-2 hover:bg-white/[0.05] rounded-lg opacity-0 group-hover:opacity-100 transition-all"><MoreHorizontal className="w-4 h-4 text-white/40" /></button>
                    </motion.div>
                  ))}
                </div>
              </GlowingCard>
              ...
              <GlowingCard>
                ...
                <div className="space-y-1">
                  {activities.map((activity, idx) => (
                    <motion.div key={activity.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }} className="flex gap-3 p-3 hover:bg-white/[0.02] rounded-xl transition-all">
                      {activity.type === 'success' && <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />}
                      {activity.type === 'warning' && <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />}
                      {activity.type === 'info' && <Radio className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />}
                      <div><p className="text-xs text-white/70">{activity.message}</p><p className="text-[10px] text-white/20 mt-0.5">{activity.time}</p></div>
                    </motion.div>
                  ))}
                </div>
              </GlowingCard>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            {[
              { icon: Brain, label: 'AI Writer', color: 'purple' },
              { icon: Globe, label: 'SEO Audit', color: 'cyan' },
              { icon: Users, label: 'Audience', color: 'emerald' },
              { icon: Workflow, label: 'Automation', color: 'amber' },
            ].map((action, idx) => (
              <motion.button key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + idx * 0.1 }} whileHover={{ scale: 1.02 }} className="p-5 bg-white/[0.02] border border-white/[0.06] rounded-2xl text-left hover:bg-purple-500/5 hover:border-purple-500/20 transition-all group">
                <div className="p-2.5 bg-purple-500/10 rounded-xl w-fit mb-3"><action.icon className="w-5 h-5 text-purple-400" /></div>
